...
public class Printtokens{
	...
	{ 
	  ...
	  StringBuilder sb = new StringBuilder();
	   ...
	   if(ch =='"')id=1;    /* prepare for string */ // Fault 5 fixed. ORIGINAL: if(ch =='"')id=2;     FIXED: if(ch =='"')id=1;
       if(ch ==59)id=2;    /* prepare for comment */ // Fault 6 fixed. ORIGINAL: if(ch ==59)id=1;     FIXED: if(ch ==59)id=2;
	...
	{
	  if (str.length() == 2 && str.charAt(0)=='#' && Character.isLetter(str.charAt(1))) // Fault 4 fixed. ORIGINAL: str.length() > 2 || str.charAt(0)=='#'     FIXED: str.length() == 2 && str.charAt(0)=='#'
	     return true;
	  else  
	     return false;
	}
	...
	/* OUTPUT:      a BOOLEAN value      */
	/*************************************/
	static boolean is_num_constant(String str) // Fault 10 fixed. ORIGINAL: Wasn't checking in proper bounds     FIXED: Now checks in the correct bounds
	{
	...
	static boolean is_str_constant(String str)
	{
	  ...
	  if ( str.charAt(0) =='"')
	     ...
	     return false;	// Fault 9; Should return false
	    }
	  else
	  ...
	  if ( Character.isLetter(str.charAt(0)) ) 
	     {
		    ...
		    return true; // Fault 1 fixed. ORIGINAL: return false;     SHOULD BE: return true;
	     }
	  else
		return false; // Fault 2 fixed. ORIGINAL: return true;     SHOULD BE: return false;
	}
	...
	{
		if      (str.equals("(")) // Fault 3 fixed. ORIGINAL: str.equals(")")     SHOULD BE: str.equals("(");
	    {
	...
	static boolean is_spec_symbol(char c)
	{
	    ...
	    // if (c == '/') // Fault 8 fixed. '/' is not a special symbol.
	    // {
	    //     return true;
	    // }
	    if (c == '`')
	    ...
	    if (c == '\'') // Fault 7 fixed. Now inclues ' in the special symbols.
	    {
	        return true;
	    }
