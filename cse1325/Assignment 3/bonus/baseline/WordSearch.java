    ...
    public void solve() {
        ...
        for (int i = 0; i < NUM_THREADS; i++)
        {
            ...
            threads[i] = new Thread(() -> {
                int firstPuzzle, lastPuzzlePlusOne;
                while (true) 
                {
                    synchronized (lock) 
                    {
                        firstPuzzle = nextThreadID * (NUM_PUZZLES / NUM_THREADS);
                        nextThreadID++;
                        lastPuzzlePlusOne = (nextThreadID == NUM_THREADS) ? NUM_PUZZLES : (nextThreadID * (NUM_PUZZLES / NUM_THREADS));
                    }

                    if (firstPuzzle < NUM_PUZZLES)
                    {
